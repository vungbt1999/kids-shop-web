...
export const transformImage = (image?: ImageFragment) => {
  ...
  if (image && Object.keys(image).length > 0) {
    imageObjData.id = image.id;
    imageObjData.url = image.url;
    imageObjData.alternativeText = 'alternativeText';
    imageObjData.formats = image.metadata;
  }

  return imageObjData;
};

export const transformListImage = (images?: ImageFragment[]) => {
  const newImage = images || [];
  if (newImage.length <= 0) return [{ ...transformImage(undefined) }];
  return newImage.map((item) => transformImage(item));
};

export const transformDisCountPercent = (price: number, promotionPrice?: number | null) => {
  if (!promotionPrice) return null;
  return `${Math.round(promotionPrice / price / 100)}%`;
};
